...
class UNet(nn.Module):
    def __init__(self,n_classes,base_channels=64,level=5,padding=0,norm_layer=None,bilinear=True,in_channels=3):
    ...
    def build_encoder(self):
        return Unet_Encoder(in_channels=self.in_channels, base_channels=self.base_channels, level=self.level, padding=self.padding)

    def build_decoder(self):
        decoder = nn.ModuleList()
        # 有 self.level-1 个上采样块
...
if __name__ == "__main__":
    ipt = torch.rand(1, 3, 128, 384)
    # unet1=UNet(2,base_channels=32,level=4)
    # opt = unet1(ipt)
    # print(opt.shape)
    unet2 = UNet(2, base_channels=16, level=4, padding=1)
    output = unet2(ipt)
    print(output.shape)
